...
const USER_COLUMNS: &str = "id,name,enabled,callback_url,created_at,updated_at";
...
pub async fn create(
    ...
    let user: User = sqlx::query_as(&format!("INSERT INTO users(id,name,callback_url,webhook_secret_encrypted) VALUES($1,$2,$3,$4) RETURNING {USER_COLUMNS}"))
        .bind(id).bind(name).bind(callback).bind(encrypted).fetch_one(&mut *tx).await?;
...
async fn patch_user(state: &AppState, id: Uuid, input: Update) -> Result<User> {
    if input.name.is_none() && input.callback_url.is_none() && input.enabled.is_none() {
